...
export default function SellerProductDetailPage() {
    ...
    if (!id) {
        return (
            <div className="flex flex-col min-h-screen bg-[#f4f6fa] pb-28 md:pb-12">
                <div className="mx-auto w-full max-w-[1488px] px-4 md:px-6 pt-4 md:pt-6 flex flex-col gap-4">
                    <BackBtn />
                    <div className="rounded-3xl border border-[#E31E24]/20 bg-[#E31E24]/5 p-6 text-center">
                        <p className="text-[15px] font-bold text-[#E31E24]">{t("errorNotFound")}</p>
                    </div>
                </div>
            </div>
        );
    ...
    if (loading) {
        return (
            <div className="flex flex-col min-h-screen bg-[#f4f6fa] pb-28 md:pb-12">
                <div className="mx-auto w-full max-w-[1488px] px-4 md:px-6 pt-4 md:pt-6 flex flex-col gap-6">
    ...
    return (
        <div className="flex flex-col min-h-screen bg-[#f4f6fa] pb-28 md:pb-12">
            <div className="mx-auto w-full max-w-[1488px] px-4 md:px-6 pt-4 md:pt-6 flex flex-col gap-5">
                ...
                <div className="grid gap-5 lg:grid-cols-[420px_1fr]">
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        {images.length > 0 ? (
                            ...
                            <div className="flex items-center justify-center aspect-square bg-slate-50 rounded-[7%] text-slate-300">
                            ...
                            <div className="flex gap-2 flex-wrap">
                                {images.map((img, i) => (
                                    <button
                                        ...
                                        className={cn(
                                            "relative w-[72px] h-[72px] rounded-xl overflow-hidden border-2 bg-white transition-all",
                                            i === activeImageIndex
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="flex items-start gap-3 flex-wrap">
                            <h1 className="text-xl md:text-2xl font-black text-slate-900 leading-tight flex-1">
                                {product.title}
                            </h1>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div className="rounded-2xl bg-white border border-slate-100 p-4 shadow-sm">
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">{t("price")}</p>
                                <p className="text-lg font-black text-[#E31E24]">
                                    {product.price.toLocaleString("ru-RU")}
                                    <span className="text-sm font-bold text-slate-400 ml-1">UZS</span>
                                </p>
                            </div>
                            <div className="rounded-2xl bg-white border border-slate-100 p-4 shadow-sm">
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">{t("stock")}</p>
                                <p className="text-lg font-black text-[#1B4D91]">
                                    {product.stock}
                                    <span className="text-sm font-bold text-slate-400 ml-1">{t("pcs")}</span>
                                </p>
                            </div>
                        </div>

                        {/* Meta */}
                        <div className="rounded-2xl bg-white border border-slate-100 p-4 shadow-sm space-y-2.5">
                            {product.sku && (
                                <div className="flex items-center gap-2.5 text-[13px]">
                                    <Hash className="size-3.5 text-[#1B4D91] shrink-0" />
                                    <span className="font-semibold text-slate-400 shrink-0">{t("article")}:</span>
                                    <span className="font-black text-[#1B4D91] tracking-wide">{product.sku}</span>
                                </div>
                            )}
                            <div className="flex items-center gap-2.5 text-[13px]">
                                <Hash className="size-3.5 text-slate-400 shrink-0" />
                                <span className="font-semibold text-slate-400 shrink-0">{t("productId")}:</span>
                                <span className="font-mono text-[11px] text-slate-500 break-all">{product.id}</span>
                            </div>
                            <div className="flex items-center gap-2.5 text-[13px]">
                                <Calendar className="size-3.5 text-slate-400 shrink-0" />
                                <span className="font-semibold text-slate-400 shrink-0">{t("createdAt")}:</span>
                        ...
                        <div className="rounded-2xl bg-white border border-slate-100 p-4 shadow-sm">
                            ...
                            {product.description ? (
                                ...
                                <p className="text-[13px] text-slate-400 italic">{t("noDescription")}</p>
                            )}
                        </div>
                        ...
                        {specs.length > 0 && (
                            <div className="rounded-2xl bg-white border border-slate-100 p-4 shadow-sm">
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-3">{t("specifications")}</p>
                ...
                <div className="rounded-3xl bg-white border border-slate-100 shadow-sm p-6">
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                        ...
                        {isApproved && (
                            <>
                                <Link
                                    href={product?.slug ? `/product/${product.slug}` : `/product/${id}`}
            ...
            <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
                <DialogContent>
                    ...
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setDeleteDialogOpen(false)} disabled={deleteLoading} className="rounded-xl">
                            {t("cancel")}
                        </Button>
                        <Button variant="destructive" disabled={deleteLoading} onClick={handleDelete} className="rounded-xl gap-1.5">
                            <Trash2 className="size-4" />
            ...
            <Dialog open={requestDialogOpen} onOpenChange={setRequestDialogOpen}>
                <DialogContent>
                    ...
                    />
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setRequestDialogOpen(false)} disabled={requestLoading} className="rounded-xl">
                            {t("cancel")}
                        </Button>
                        <Button
                            variant="destructive"
