...
import { getProducts, getParentCategories } from "@/lib/api/products";
...
import { getCategoryImage, getCategoryShortName } from "@/lib/constants/categories";
...
export default function CatalogPage() {
  ...
  useEffect(() => {
    getParentCategories().then(setCategories).catch(console.error);
  }, []);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const res = await getProducts(page, 12, expandedSearch, undefined, filters.minPrice, filters.maxPrice, filters.sortBy);
        if (!cancelled) {
          setError("");
          setData(res);
        }
        ...
        <div className="mx-auto flex flex-col gap-6 px-4 md:px-6 max-w-md md:max-w-none pb-24 md:pb-0 pt-6 md:pt-6">
          ...
          <div className="flex gap-6 items-start">
            ...
            <div className="flex-1 w-full min-w-0 pb-20 md:pb-0">
              ...
              {!loading && !error && (
                <>
                  <div className={cn("grid grid-cols-2 gap-2", isDesktop ? "hidden" : !isMobileSearchActive ? "grid" : "hidden")}>
                    {categories.map((cat) => (
                      <CategoryCard
                        key={cat.id}
                        id={cat.id}
                        name={getCategoryShortName(cat.code, locale, cat.name)}
