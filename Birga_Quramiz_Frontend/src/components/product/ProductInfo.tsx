"use client";

import type { Product } from "@/types";
import { Star } from "lucide-react";
import CategoryCard from "./CategoryCard";

type ProductInfoProps = {
  product: Product;
  onOpenSpecs: () => void;
};

export default function ProductInfo({
  product,
  onOpenSpecs
}: ProductInfoProps) {
  return (
    <div className="flex flex-col gap-6 mt-2 md:mt-0 relative z-10 w-full min-w-0 order-2 md:order-none col-span-1">
      {/* Brand Badge & Title */}
      <div className="flex flex-col gap-3">
        {(product.brand || product.seller?.company) && (
          <div className="flex items-center">
            {product.brand ? (
              <a 
                href={`/brands/${product.brand.slug}`}
                className="px-3 py-1.5 bg-[#F6F6F9] hover:bg-[#EEF0F3] transition-colors rounded-lg text-[13px] font-semibold text-slate-900 border border-slate-100"
              >
                {product.brand.name}
              </a>
            ) : (
              <a 
                href={`/catalog?brand=${encodeURIComponent(product.seller!.company)}`}
                className="px-3 py-1.5 bg-[#F6F6F9] hover:bg-[#EEF0F3] transition-colors rounded-lg text-[13px] font-semibold text-slate-900 border border-slate-100"
              >
                {product.seller!.company}
              </a>
            )}
          </div>
        )}
        <h1 className="text-[20px] lg:text-[24px] font-bold text-[#242424] leading-[1.2] mt-1">
          {product.name}
        </h1>

        {/* Rating and Questions row */}
        {/* 
        <div className="flex items-center gap-4 text-[13px]">
          <div 
            className="flex items-center gap-1.5 cursor-pointer hover:opacity-80 transition-opacity"
            onClick={() => document.getElementById('reviews-section')?.scrollIntoView({ behavior: 'smooth' })}
          >
            <div className="flex items-center gap-0.5">
              <Star className="size-3.5 fill-[#f9b41b] text-[#f9b41b]" />
              <span className="font-bold text-[#242424]">
                {product.rating && product.rating > 0 ? product.rating.toFixed(1) : "0.0"}
              </span>
            </div>
            <span className="text-slate-400">·</span>
            <span className="text-[#a0a0a0] font-medium border-b border-[#a0a0a0]/30 hover:border-[#a0a0a0] transition-colors">
              {product.reviewsCount || 0} оценок
            </span>
          </div>
          
          <div className="flex items-center gap-1.5 text-[#a0a0a0] font-medium cursor-pointer hover:opacity-80 transition-opacity">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path></svg>
            <span className="border-b border-[#a0a0a0]/30 hover:border-[#a0a0a0] transition-colors">
              0 вопросов
            </span>
          </div>
        </div>
        */}
      </div>

      {/* Standardized Specs Table */}
      <div className="w-full">
        <table className="w-full border-collapse">
          <tbody>
            {(product.specifications ? Object.entries(product.specifications).slice(0, 4) : [
              { label: "Артикул", value: product.sku || product.id.slice(0, 8).toUpperCase(), id: true },
              ...(product.category ? [{ label: "Категория", value: product.category.name }] : []),
              { label: "В наличии", value: product.stock > 0 ? `${product.stock} шт.` : "Нет в наличии" },
              { label: "Бренд", value: product.brand?.name || product.seller?.company || "Неизвестно" },
            ]).map((spec: [string, string] | { label: string; value: string; id?: boolean }, i) => {
              const label = Array.isArray(spec) ? spec[0] : spec.label;
              const value = Array.isArray(spec) ? spec[1] : spec.value;
              const isId = !Array.isArray(spec) && spec.id;

              return (
                <tr key={i} className="group">
                  <th className="py-2 pr-4 text-left font-normal border-b border-transparent group-last:border-none">
                    <div className="flex items-end gap-1">
                      <span className="text-[#a0a0a0] text-[14px] leading-tight shrink-0">{label}</span>
                      <div className="w-full border-b border-dotted border-slate-200 mb-1" />
                    </div>
                  </th>
                  <td className="py-2 pl-2 text-right md:text-left border-b border-transparent group-last:border-none">
                    <span className="text-[#242424] text-[14px] font-medium leading-tight inline-flex items-center gap-1.5">
                      {value}
                      {isId && (
                        <button 
                          onClick={() => {
                            navigator.clipboard.writeText(value);
                          }}
                          className="text-[#a0a0a0] hover:text-slate-600 p-0.5"
                        >
                          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>
                        </button>
                      )}
                    </span>
                  </td>
                </tr>
              );
            })}
          </tbody>
        </table>
      </div>

      {/* Quick specs overview (links to drawer) */}
      <div className="w-full">
        <button 
          onClick={onOpenSpecs}
          className="cursor-pointer inline-flex items-center px-4 py-1.5 bg-[#F6F6F9] hover:bg-[#EEF0F3] rounded-full text-[13px] font-semibold text-[#242424] transition-colors"
        >
          Характеристики и описание
        </button>
      </div>

      {/* Return Policy and Separator */}
      <div className="flex flex-col gap-4">
        <div className="flex items-center gap-3 py-1">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-slate-800">
            <polyline points="23 4 23 10 17 10"></polyline>
            <polyline points="1 20 1 14 7 14"></polyline>
            <path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"></path>
          </svg>
          <span className="text-[#242424] text-[15px] font-bold">14 дней на возврат</span>
        </div>
        
        <div className="w-full h-px bg-slate-100" />
      </div>

      {/* Category link relocates here or remains right after */}
      <CategoryCard product={product} />
    </div>
  );
}
