...
export default function SpecsDrawer({
  ...
  const specifications = product.specifications ? Object.entries(product.specifications).map(([label, value]) => ({ label, value })) : [
    { label: "Бренд", value: product.seller?.company || "Birga-Quramiz" },
    { label: "Артикул", value: product.sku || product.id.slice(0, 8).toUpperCase() },
    { label: "Категория", value: product.category?.name || "Строительные материалы" },
    { label: "Страна производства", value: "Узбекистан" },
  ];
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-md md:max-w-lg p-0 flex flex-col bg-white border-l-0 shadow-[-10px_0_40px_rgba(0,0,0,0.1)] gap-0">
        <SheetHeader className="px-6 py-5 border-b border-slate-100 bg-white z-10 sticky top-0">
          <SheetTitle className="text-[20px] font-bold text-[#242424] leading-tight">Характеристики и описание</SheetTitle>
        </SheetHeader>
        
        <div className="flex-1 overflow-y-auto w-full px-6 py-6 pb-[140px]">
          {/* Detailed Description */}
          <div className="mb-10">
            <h3 className="text-[16px] font-bold text-[#242424] mb-4">Описание</h3>
            <p className="text-[14px] leading-relaxed text-slate-600">
              {product.description || "Подробного описания пока нет, но мы работаем над этим."}
            </p>
          </div>

          {/* Specifications */}
          <div className="flex flex-col gap-6">
            <h3 className="text-[16px] font-bold text-[#242424] -mb-2">Характеристики</h3>
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-slate-100 p-5 shadow-[0_-10px_30px_rgba(0,0,0,0.03)] flex flex-col gap-3">
          <div className="flex items-center justify-between mb-1">
            <span className="text-[13px] font-medium text-slate-400">Итого:</span>
            <div className="flex items-baseline gap-1.5">
              <span className="text-[26px] font-black text-[#242424]">
                {(price * (quantity || 1)).toLocaleString("ru-RU")} 
              </span>
              <span className="text-[14px] font-bold text-[#242424]">{t("currencyUzs") || "UZS"}</span>
            </div>
          </div>
